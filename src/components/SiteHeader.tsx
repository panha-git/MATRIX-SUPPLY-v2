...
const logoutButtonClass = [
  "hidden rounded-xl border border-line px-3 py-2 text-xs font-bold",
  "text-muted-ink hover:border-primary/30 hover:text-primary sm:block",
].join(" ");
...
export function SiteHeader() {
  ...
  const mobileLinkClass = (href: string) =>
    [
      ...
      isActive(href) ? "bg-primary-soft text-primary" : "hover:bg-[#f5f7f5]",
    ].join(" ");
    ...
    <header className="sticky top-0 z-50 bg-white/95 shadow-[0_1px_0_rgba(20,45,27,.08)] backdrop-blur-xl">
      <div className="bg-primary-dark text-white">
        <div className="container-shell flex h-7 items-center justify-between text-[11px] text-white/75">
          <span>
            {isCustomer
              ? "Customer marketplace · Active supplier products"
              : "Supplier workspace · Your products and profile"}
          </span>
          <span className="hidden items-center gap-4 sm:flex">
            <span className="max-w-60 truncate">{user?.gmail}</span>
      ...
      <div className="container-shell flex h-[72px] items-center gap-5">
        ...
        {isCustomer && (
          <form
            ...
            className="hidden min-w-[240px] max-w-xl flex-1 lg:flex"
          >
        ...
        <div className="ml-auto flex items-center gap-2 xl:ml-0">
          <button onClick={logout} className={logoutButtonClass}>
            Logout
          </button>
          <button
